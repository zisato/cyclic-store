...
describe('Update Product acceptance test', () => {
  ...
  test('When not existing product id returns 404 status code', async () => {
    ...
    // When
    const response = await request(server).patch(route).set('authorization', `Bearer ${userId}`).send(requestBody)
  ...
  test('When valid request returns 204 status code', async () => {
    ...
    const route = getRoute(productId)
    const requestBody = givenValidRequestBody({ name: newProductName })
    await givenExistingSeller(userId)
